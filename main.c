...
char kit[][256] = {" -> Check if the device exists (BLKNAME)"," -> Dump one block"," -> Dump MBR"," -> Dump device (MASTER)"," -> Dump device (FAT16)"," -> Dump device (exFAT)"," -> Dump device (custom size)"," -> Bruteforce device names"};
int item_count = 8;
int smodes = 6;
int szodes = 4;
int smd = 0;
...
if (omd == 4) psvDebugScreenPrintf("       DEV:CUSTOM");
psvDebugScreenPrintf("      LOC:%s\n\n", ddev[smode]);
psvDebugScreenSetFgColor(COLOR_GREEN);
psvDebugScreenPrintf("                           NDP v6");
...
if (omd < 3) psvDebugScreenPrintf("         PRESS [SELECT] TO PERFORM A FULL DEVICE DUMP");
if (omd == 4) psvDebugScreenPrintf("                  PRESS [SELECT] TO GO BACK");
if (omd == 4) psvDebugScreenPrintf("             PRESS [START] TO ACCEPT AND CONTINUE");
psvDebugScreenSetFgColor(COLOR_WHITE);
if (omd == 4) psvDebugScreenPrintf("                     ENTER DEVICE SIZE:");
psvDebugScreenSetFgColor(COLOR_GREEN);
if (omd == 4) psvDebugScreenPrintf("         DEVICE SIZE (BLOCKS): 0x%X\n", ocdevsz);
psvDebugScreenSetFgColor(COLOR_YELLOW);
if (omd == 4) psvDebugScreenPrintf("          DEVICE SIZE (FINAL): 0x%X\n", (ocdevsz * 0x200));
psvDebugScreenSetFgColor(COLOR_RED);
...
sceAppMgrLoadExec("app0:fapordev", NULL, NULL);}
int brutefap(){
psvDebugScreenPrintf("Bruteforcing, please be patient.\n");
const char hw[7][32] = {"int", "ext", "gcd", "mcd", "uma", "usd", "xmc"};
...
if (omd == 2) ndpWorkDevBRBasedFull(mcdblk, "mc", 0);
if (omd == 4) omd = 3;
smn();
sceKernelDelayThread(0.35 * 1000 * 1000);}
